...
course_id
-----------
  The ID of the course run that the user is enrolling in (for example, 
  ...
  **History**: In October 2014, identifiers for new courses began to use the 
  format shown above. The format for the course_id prior to this change used 
  slash (/) characters as separators. For example, MITx/6.002x/2012_Fall. 
  The URL format for a course with a course_id in this format was 
...
course_id
-----------
  The course identifier, in the format ``{org}+{course}+{run}``. For 
  example, edX+DemoX+Demo_2014. 
...
-----------
  ...
  As an example, this module_id:
    ...
    i4x://edX+DemoX+Demo_2014+type@problem+block@303034da25524878a2e66fb57c91cf85
...
course_id
-----------
  The course that this row applies to, in the format 
  ``{org}+{course}+{run}``. For example, edX+DemoX+Demo_2014. 
  ...
  **History**: Prior to October 2014, course_id values used the format
